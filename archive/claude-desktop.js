...
/**
 * Calls Claude API with MCP format messages
 * 
 * @param {Array} messages Array of MCP-formatted messages
 * @param {string} model Claude model to use
 * @returns {object} Parsed JSON response or error object
 */
async function callClaude(messages, model = 'claude-3-7-sonnet-20250219') {
  ...
  // Claude already supports MCP format, so we don't need to convert the messages
  const payload = {
    model: model,
    messages: messages,
    temperature: 0.5,
    max_tokens: 2048,
    system: "Respond with valid JSON only. No other text, explanations, or markdown formatting outside the JSON."
  };
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Claude API error: ${res.status}. Details: ${errorText.substring(0, 200)}`);
    }
    ...
    if (typeof rawContent !== 'string' || rawContent.trim() === '') {
        console.warn('Received empty or non-string content from Claude:', rawContent);
        return {
            error: "Claude returned empty or invalid content",
    ...
    if (parsedJson !== null) {
        ...
        console.error('Failed to parse JSON from Claude response despite robust attempt. Raw content:', rawContent);
        return {
    ...
    console.error('Error calling Claude or processing response:', error);
...
\`\`\`` },
  ...
  try {
    const [res1, res2] = await Promise.allSettled([
      callClaude(messages1),
      callClaude(messages2)
    ]);
...
Original: "${headline}"` }
    ...
    const synthesis = await callClaude(messages3, 'claude-3-7-sonnet-20250219');
